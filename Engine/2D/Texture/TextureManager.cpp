...
#pragma region Upload
	...
	dx->GetDev()->CreateCommittedResource(
		...
		D3D12_RESOURCE_STATE_GENERIC_READ,		//	CPUから書き込み可能、GPUは読み取り専用
		nullptr,
		IID_PPV_ARGS(asyncUploadBuff_.back().ReleaseAndGetAddressOf()));
#pragma endregion

	//	転送
	uint8_t* mapforImg = nullptr;
	result = asyncUploadBuff_.back().Get()->Map(0, nullptr, (void**)&mapforImg);	//	map
	...
	asyncUploadBuff_.back().Get()->Unmap(0, nullptr);	//	unmap
...
#pragma region CopyCommand
	...
	//	グラフィックボード上のコピー元アドレス
	D3D12_TEXTURE_COPY_LOCATION src{};
	src.pResource = asyncUploadBuff_.back().Get();
	src.Type = D3D12_TEXTURE_COPY_TYPE_PLACED_FOOTPRINT;
	src.PlacedFootprint = footprint;
