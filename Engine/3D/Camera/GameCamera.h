...
private:
	...
	MyMath::Quaternion startFront_;
	MyMath::Quaternion endFront_;
	//	最大時の移動終了時間
	...
	//	一番カメラの向きから近い敵の位置
	MyMath::Vector3D* pEnemyPos_ = nullptr;
	//	プレイヤーの位置
	MyMath::Vector3D* pPlayerPos_ = nullptr;
...
#pragma region Setter
	...
	*/
	void SetEnemyPos(MyMath::Vector3D* pos);
	/**
	* @fn SetPlayerPos(Vector3D*)
	* プレイヤーの位置を設定するための関数
	* @param pos プレイヤーの位置のポインタ
	*/
	void SetPlayerPos(MyMath::Vector3D* pos);
	/**
	* @fn SetIsActive(bool)
	* 実行中かどうかの設定をする溜めの関数
