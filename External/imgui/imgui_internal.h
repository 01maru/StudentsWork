...
#ifdef _WIN32
#define IM_NEWLINE                      "\r\n"   // Play it nice with Windows users (RSSetVPandSR: since 2018-05, Notepad finally appears to support Unix-style carriage returns!)
#else
#define IM_NEWLINE                      "\n"
#endif
...
struct IMGUI_API ImGuiMenuColumns
{
    ...
    ImU16       OffsetLabel;        // Offsets are locked in RSSetVPandSR()
...
struct ImGuiViewportP : public ImGuiViewport
{
    ...
    void    UpdateWorkRect()            { WorkPos = CalcWorkRectPos(WorkOffsetMin); WorkSize = CalcWorkRectSize(WorkOffsetMin, WorkOffsetMax); } // RSSetVPandSR public fields
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImGuiViewportP*         Viewport;                           // Always set in Begin(). Inactive windows may have a NULL value here if their viewport_ was discarded.
    ...
    ImRect                  InnerClipRect;                      // == InnerRect shrunk by WindowPadding*0.5f on each side, clipped within viewport_ or parent clip rect.
...
namespace ImGui
{
    ...
    IMGUI_API ImDrawList*   GetBackgroundDrawList(ImGuiViewport* viewport);                     // get background draw list for the given viewport_. this draw list will be the first rendering one. Useful to quickly draw shapes/text behind dear imgui contents.
    IMGUI_API ImDrawList*   GetForegroundDrawList(ImGuiViewport* viewport);                     // get foreground draw list for the given viewport_. this draw list will be the last rendered one. Useful to quickly draw shapes/text over dear imgui contents.
